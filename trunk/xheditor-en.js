...
var htmlAbout='<div style="width:200px;word-wrap:break-word;word-break:break-all;"><p><span style="font-size:20px;color:#1997DF;">xhEditor</span><br />ver: v1.0.0 beta build 100110</p><p>xhEditor is a simple,small,fast WYSWYG(What You See What You Get) XHTML editor based by jQuery.</p><p><a href="http://xheditor.com/" target="_blank">http://xheditor.com/</a></p></div>';
...
$.xheditor=function(textarea,options)
{
	...
	{
		var s1='<span class="xheUpload"><input type="text" style="visibility:hidden;" /><input type="button" value="Upload" class="xheBtn" />',jUpload;
		...
		{//系统默认ajax上传
			...
			jFile.change(function(){
				...
				if(sFile!='')
				{
					...
					{
						bShowPanel=false;//防止按钮面板被关闭
						var modal=_this.showModal('Upload file','<div style="margin:22px 0;text-align:center;line-height:30px;">File uploading,please wait...<br /><img src="'+skinPath+'img/loading.gif"></div>',320,150,function(){bShowPanel=true;});
	...
	this.showModal=function(title,content,w,h,onRemove)
	{
		...
		jModal=$('<div class="xheModal" style="width:'+(w-1)+'px;height:'+h+'px;margin:-'+Math.ceil(h/2)+'px 0px 0px -'+Math.ceil(w/2)+'px;">'+(_this.settings.modalTitle?'<div class="xheModalTitle"><span class="xheModalClose" title="Close (Esc)"></span>'+title+'</div>':'')+'<div class="xheModalContent"></div></div>').appendTo('body');

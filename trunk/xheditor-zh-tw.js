...
{
	if(bMobile)return false;//手機瀏覽器不初始化編輯器
	var arrSuccess=[];
	this.each(function(){
...
var xheditor=function(textarea,options)
{
	...
	{
		if( (url.match(/^(\w+):\/\//i) && !url.match(/^https?:/i)) || url.match(/^#/i) )return url;//非http和https協議，或者頁面錨點不轉換

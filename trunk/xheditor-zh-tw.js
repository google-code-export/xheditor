	...
	return this.each(function(){
		...
		if(bInit)//初始化
		{
			if(!this.objxhe)
			{
				...
				if(tOptions)
				{
					try{eval("tOptions=" + tOptions);}catch(ex){};
					options=$.extend({},tOptions,options );
				}
				var editor=new $.xheditor(this,options);
				if(editor.init())this.objxhe=editor;			
				else editor=null;	
			}
		}
		else//卸載
...
var xCount=0,isIE=$.browser.msie,isMozilla=$.browser.mozilla,isSafari=$.browser.safari,bShowPanel=false,bClickCancel=true;
...
var arrFontname=[{n:'細明體',c:'mingliu'},{n:'新細明體',c:'PMingLiu'},{n:'黑體',c:'SimHei'},{n:'楷體',c:'KaiTi_GB2312'},{n:'微軟雅黑',c:'Microsoft YaHei'},{n:'Arial'},{n:'Arial Narrow'},{n:'Arial Black'},{n:'Comic Sans MS'},{n:'Courier New'},{n:'System'},{n:'Times New Roman'},{n:'Tahoma'},{n:'Verdana'}];
...
var menuAlign=[{s:'左對齊',v:'justifyleft',t:'左對齊'},{s:'居中',v:'justifycenter',t:'居中'},{s:'右對齊',v:'justifyright',t:'右對齊'},{s:'兩端對齊',v:'justifyfull',t:'兩端對齊'}],menuList=[{s:'數字列表',v:'insertOrderedList',t:'數字列表'},{s:'符號列表',v:'insertUnorderedList',t:'符號列表'}];
var htmlPastetext='<div>使用鍵盤快捷鍵(Ctrl+V)把內容粘貼到方框裡，按 確定</div><div><textarea id="xhePastetextValue" wrap="soft" spellcheck="false" style="width:300px;height:100px;" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="確定" /></div>';
...
var htmlTable='<div>行數列數：<input type="text" id="xheTableRows" style="width:40px;" value="3" /> x <input type="text" id="xheTableColumns" style="width:40px;" value="2" /></div><div>標題單元：<select id="xheTableHeaders"><option selected="selected" value="">無</option><option value="row">第一行</option><option value="col">第一列</option><option value="both">第一行和第一列</option></select></div><div>寬度高度：<input type="text" id="xheTableWidth" style="width:40px;" value="200" /> x <input type="text" id="xheTableHeight" style="width:40px;" value="" /></div><div>邊框大小：<input type="text" id="xheTableBorder" style="width:40px;" value="1" /></div><div>表格間距：<input type="text" id="xheTableCellSpacing" style="width:40px;" value="1" /> 表格填充：<input type="text" id="xheTableCellPadding" style="width:40px;" value="1" /></div><div>對齊方式：<select id="xheTableAlign"><option selected="selected" value="">默認</option><option value="left">左對齊</option><option value="center">居中</option><option value="right">右對齊</option></select></div><div>表格標題：<input type="text" id="xheTableCaption" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="確定" /></div>';
var htmlAbout='<div style="width:200px;word-wrap:break-word;word-break:break-all;"><p><span style="font-size:20px;color:#1997DF;">xhEditor</span><br />版本：v1.0.0 beta build 100110</p><p>xhEditor是一個基於jQuery開發的跨平台開源迷你XHTML編輯器組件。</p><p><a href="http://xheditor.com/" target="_blank">http://xheditor.com/</a></p></div>';
...
var arrTools={GStart:{},GEnd:{},Separator:{},Cut:{t:'剪切 (Ctrl+X)'},Copy:{t:'複製 (Ctrl+C)'},Paste:{t:'粘貼 (Ctrl+V)'},Pastetext:{t:'粘貼文本'},Blocktag:{t:'段落標籤'},Fontface:{t:'字體'},FontSize:{t:'字號'},Bold:{t:'加粗 (Ctrl+B)',s:'Ctrl+B'},Italic:{t:'斜體 (Ctrl+I)',s:'Ctrl+I'},Underline:{t:'下劃線 (Ctrl+U)',s:'Ctrl+U'},Strikethrough:{t:'中劃線 (Ctrl+S)',s:'Ctrl+S'},FontColor:{t:'字體顏色'},BackColor:{t:'背景顏色'},Removeformat:{t:'刪除文字格式'},Align:{t:'對齊'},List:{t:'列表'},Outdent:{t:'減少縮進 (Shift+Tab)',s:'Shift+Tab'},Indent:{t:'增加縮進 (Tab)',s:'Tab'},Link:{t:'超鏈接'},Unlink:{t:'取消超鏈接'},Img:{t:'圖片'},Flash:{t:'Flash動畫'},Media:{t:'多媒體文件'},Emot:{t:'表情'},Table:{t:'表格'},Source:{t:'源代碼'},Preview:{t:'預覽'},Fullscreen:{t:'全屏編輯 (Esc)',s:'Esc'},About:{t:'關於 xhEditor'}};
var toolsThemes={
...
$.xheditor=function(textarea,options)
{
	...
	if(_this.settings.plugins)arrTools=$.extend({},arrTools,_this.settings.plugins);
	...
	this.init=function()
	{
		...
		iframeHTML+='<base'+(_this.settings.baseUrl!=baseURL?' href="'+_this.settings.baseUrl+'"':'')+' /></head><body spellcheck="false" dir="ltr" class="editMode'+bodyClass+'"></body></html>';
	...
	this.processHTML=function(sHtml,mode)
	{
		...
		if(mode=='write')
		{//write
			if(_this.settings.keepValue)//保存屬性值:src,href
			{
			...
			sHtml = sHtml.replace(/<(\/?)del( [^>]+)?>/ig,'<$1strike$2>');//編輯狀態統一轉為strike
			if(isMozilla)
			{
			...
			if(!isSafari)
			{
				...
				sHtml = sHtml.replace(/<(span)(?:\s+[^>]+)? style="((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S])*?)<\/\1>/ig,style2font);//最裡層
				sHtml = sHtml.replace(/<(span)(?:\s+[^>]+)? style="((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig,style2font);//第2層
				sHtml = sHtml.replace(/<(span)(?:\s+[^>]+)? style="((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,style2font);//第3層
			}
		}
		else
		{//read
			if(_this.settings.keepValue)//恢復屬性值
			{
				function restoreValue(all,tag,attrs,n,q,v)
				{
					attrs=attrs.replace(new RegExp('\\s+'+n+'\\s*=\\s*(["\']?)[^\\1\\s]*\\1','ig'),' '+n+'="'+v.replace(/\$/g,'$$$$')+'"');
					return '<'+tag+attrs+'>';
				}
				sHtml = sHtml.replace(/<(\w+(?:\:\w+)?)(\s+[^>]*?_xhe_(src|href)\s*=\s*(['"]?)\s*([^\4\s]*)\s*\4[^>]*)>/ig,restoreValue);
			}
			if(isSafari)
			{
				sHtml = sHtml.replace(/("|;)\s*font-size\s*:\s*([a-z-]+)(;?)/ig,function(all,pre,sname,aft){
				...
				sHtml = sHtml.replace(/<(span)(\s+[^>]+|)? class="Apple-style-span"(\s+[^>]+|)?>(((?!<\1(\s+[^>]+)?>)[\s\S])*?)<\/\1>/ig,replaceAppleSpan);//最裡層
				sHtml = sHtml.replace(/<(span)(\s+[^>]+|)? class="Apple-style-span"(\s+[^>]+|)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig,replaceAppleSpan);//第2層
				sHtml = sHtml.replace(/<(span)(\s+[^>]+|)? class="Apple-style-span"(\s+[^>]+|)?>(((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S]|<\1(\s+[^>]+)?>((?!<\1(\s+[^>]+)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,replaceAppleSpan);//第3層
			}
			sHtml = sHtml.replace(/(\s+)(?:_xhe_|_moz_|_webkit_)[^=]+?\s*=\s*(["']?)[^\2\s>]*\2\s*/ig,'$1');
	...
	this.cleanWord=function(sHtml)
	{
		if(sHtml.match(/mso-|MsoNormal/i))
		{
			...
			//屬性清理
			sHtml = sHtml.replace(/<(\w+[^>]*?)\s+class="?mso[^ >]*([^>]*?)>/ig, "<$1$2>");//刪除所有mso開頭的樣式
			sHtml = sHtml.replace(/<(\w+[^>]*?)\s+lang="?[^ \>]*([^>]*?)>/ig, "<$1$2>");//刪除lang屬性
			sHtml = sHtml.replace(/<(\w+[^>]*?)\s+align="?left"?([^>]*?)>/ig, "<$1$2>");//取消align=left
			//sHtml = sHtml.replace(/<(\w+[^>]*?) style="?[^">]+"?([^>]*?)>/ig, "<$1$2>");//刪除所有style
			...
			sHtml = sHtml.replace(/<(\w+[^>]*?)\s+style="?\s*([^">]+)\s*"?([^>]*?)>/ig, function(all,attr1,styles,attr2){return '<'+attr1+' style="'+styles.replace(/&quot;/ig,'')+'"'+attr2+'>';});//清理style裡的&quot;
		}
		return sHtml;
	}
	this.cleanHTML=function(sHtml)
	{
		...
		sHtml = sHtml.replace(/<(meta|title)(\s+[^>]+)?>/ig, "");
		...
		sHtml=sHtml.replace(/<\/(strong|b|u|strike|em|i)>((?:\s|<br\/?>|&nbsp;)*?)<\1>/ig,'$2');//連續相同標籤
	...
	this.toggleSource=function(state)
	{
		...
		{//轉為編輯模式
			...
			{
				_doc.designMode = 'On';
				if(isMozilla)_this._exec("inserthtml","-");//修正源代碼切換回來無法刪除文字的問題
			}
	...
	this.showMenu=function(menuitems,callback)
	{
		...
		$.each(menuitems,function(n,v){
			jItem=$('<a href="javascript:void(0);" title="'+v.t+'">'+v.s+'</a>').click(function(){_this.focus();callback(v.v);_this.hidePanel();return false;}).mousedown(function(){return false;});
			jMenu.append(jItem);
		});
	...
	this.showColor=function(callback)
	{
		...
		{
			c++;
			jItem=$('<a href="javascript:void(0);" title="'+v+'" style="background:'+v+'"><img src="'+skinPath+'img/spacer.gif" /></a>').click(function(){_this.focus();callback(v);_this.hidePanel();}).mousedown(function(){return false;});
			jLine.append(jItem);
			if(c%10==0)
			{
				jColor.append(jLine);
			...
			if(url!=''&&url!='http://')
			{
				if(jParent.length==0)
				{
					if(selHtml=='')
					{
						var txt=jText.val();
						if(!txt)txt=url;
						_this.pasteHTML('<a href="#xhe_tmpurl">'+txt+'</a>');
					}
		...
		jSave.click(function(){
			...
			if(url!=''&&url!='http://')
			{
				...
				{//批量插入
					...
					if(jAlt.val()!='')sTemplate+=' alt="'+jAlt.val()+'"';
					if(jAlign.val()!='')sTemplate+=' align="'+jAlign.val()+'"';
					if(jWidth.val()!='')sTemplate+=' width="'+jWidth.val()+'"';
					if(jHeight.val()!='')sTemplate+=' height="'+jHeight.val()+'"';
					if(jBorder.val()!='')sTemplate+=' border="'+jBorder.val()+'"';
					if(jVspace.val()!='')sTemplate+=' vspace="'+jVspace.val()+'"';
					if(jHspace.val()!='')sTemplate+=' hspace="'+jHspace.val()+'"';
					sTemplate+=' />';
					for(var i in aUrl)
					{
						url=aUrl[i];
	...
	this.showEmbed=function(sType,sHtml,sMime,sClsID,sBaseAttrs,sUploadUrl,sUploadExt)
	{
		...
		if(sUploadUrl)_this.ajaxUploadInit(jSrc,sUploadUrl,sUploadExt);
		...
		jSave.click(function(){
			...
			if(sSrc!=''&&sSrc!='http://')
			{
				...
				if(aSrc.length>1)
				{//批量插入
					var sTemplate=sBaseCode+'';
					sTemplate+=' width="'+w+'" height="'+h+'" />';
					for(var i in aSrc)
					{
						sSrc=aSrc[i];
						if(sSrc!='')_this.pasteHTML(sTemplate.replace('xhe_tmpurl',sSrc));
					}
				}
				else if(aSrc.length==1)
				{//單URL模式
					sSrc=aSrc[0];
					if(jParent.length==0)
					{
						_this.pasteHTML(sBaseCode.replace('xhe_tmpurl',sSrc+'#xhe_tmpurl')+' />');
	...
	{
		var s1='<span class="xheUpload"><input type="text" style="visibility:hidden;" /><input type="button" value="上傳" class="xheBtn" />',jUpload;
		if(tourl.substr(0,1)=='!')//自定義上傳管理頁
		{
			jUpload=$(s1+'</span>');
			jText.after(jUpload);
			$('.xheBtn',jUpload).before(jText).click(function(){
				bShowPanel=false;//防止按鈕面板被關閉
				_this.showIframeModal(tourl.substr(1),'上傳文件',function(v){
					if(v.substr(0,1)=='!')
					{
						jText.val(v.substr(1));
						jText.closest('.xheDialog').find('#xheSave').click();
					}
					else jText.val(v);
				},null,null,function(){bShowPanel=true;});
			});
		}
		else
		{//系統默認ajax上傳
			jUpload=$(s1+'<input type="file" class="xheFile" size="13" name="filedata" id="xheUploadFile" /></span>');
			jText.after(jUpload);
			$('.xheBtn',jUpload).before(jText);
			var jFile=$('#xheUploadFile',jUpload);
	...
	this.ajaxUpload=function(fromfile,tourl,callback,onError)
	{
		...
		var jIO=$('<iframe name="'+idIO+'" class="xheUploadIO" />').appendTo('body');
		var jForm=$('<form action="'+tourl+'" target="'+idIO+'" method="post" enctype="multipart/form-data" class="xheUploadForm"></form>').appendTo('body');
	...
	this.showIframeModal=function(ifmurl,title,callback,w,h,onRemove)
	{
	...
	this.showModal=function(title,content,w,h,onRemove)
	{
		...
		var jModal,jModalShadow,jOverlay,layerShadow;
		w=w?w:_this.settings.modalWidth;h=h?h:_this.settings.modalHeight;
		layerShadow=_this.settings.layerShadow;
		jModal=$('<div class="xheModal" style="width:'+(w-1)+'px;height:'+h+'px;margin:-'+Math.ceil(h/2)+'px 0px 0px -'+Math.ceil(w/2)+'px;">'+(_this.settings.modalTitle?'<div class="xheModalTitle"><span class="xheModalClose" title="關閉 (Esc)"></span>'+title+'</div>':'')+'<div class="xheModalContent"></div></div>').appendTo('body');
		jOverlay=$('<div class="xheModalOverlay"></div>').appendTo('body').mousedown(function(){return false;});
		if(layerShadow>0)jModalShadow=$('<div class="xheModalShadow" style="width:'+jModal.outerWidth()+'px;height:'+jModal.outerHeight()+'px;margin:-'+(Math.ceil(h/2)-layerShadow-2)+'px 0px 0px -'+(Math.ceil(w/2)-layerShadow-2)+'px;"></div>').appendTo('body').mousedown(function(){return false;});
		
		$('.xheModalContent',jModal).css('height',h-(_this.settings.modalTitle?$('.xheModalTitle').outerHeight():0)).html(content);
		
		function remove(){$(document).unbind('keydown',escCheck);jModal.remove();if(layerShadow>0)jModalShadow.remove();jOverlay.remove();if(onRemove)onRemove();};this.remove=remove;
	...
	{
		var jEmot=$('<div class="xheEmot"></div>'),jLine,jItem,c=0,sEmotPath=jsURL+'xheditor_emot/default/';
		jLine=$('<div></div>');
		$.each(itemEmots,function(n,v)
		{
			c++;
			jItem=$('<a href="javascript:void(0);" title="'+v.t+'"><img src="'+sEmotPath+v.s+'" /></a>').click(function(){
				_this.focus();
	...
	{
		var jDialog=$('<div class="xheDialog"></div>'),jSave=$('#xheSave',content);
		if(jSave.length==1)
		{
			content.find('input[type=text]').keypress(function(ev){if(ev.which==13){jSave.click();return false;}});
			jSave.after(' <input type="button" id="xheCancel" value="取消" />');
			$('#xheCancel',content).click(_this.hidePanel);
	...
	this.exec=function(cmd)
	{
		...
		switch(cmd)
		{
			case 'cut':
				...
				catch(ex){alert('您的瀏覽器安全設置不允許使用剪切操作，請使用鍵盤快捷鍵(Ctrl + X)來完成');};
				break;
			case 'copy':
				try{_doc.execCommand(cmd);if(!_doc.queryCommandSupported(cmd))throw 'Error';}
			...
			case 'paste':
				try{_doc.execCommand(cmd);if(!_doc.queryCommandSupported(cmd))throw 'Error';}
				catch(ex){alert('您的瀏覽器安全設置不允許使用粘貼操作，請使用鍵盤快捷鍵(Ctrl + V)來完成');}
				break;
			case 'pastetext':

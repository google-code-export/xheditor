...
var htmlAbout='<div style="font:12px Arial;width:245px;word-wrap:break-word;word-break:break-all;"><p><span style="font-size:20px;color:#1997DF;">xhEditor</span><br />v1.1.0 (build 100901)</p><p>xhEditor是基於jQuery開發的跨平台輕量XHTML編輯器，基於<a href="http://www.gnu.org/licenses/lgpl.html" target="_blank">LGPL</a>開源協議發佈。</p><p>Copyright c <a href="http://xheditor.com/" target="_blank">xhEditor.com</a>. All rights reserved.</p></div>';
...
$.xheditor=function(textarea,options)
{
	...
	this.init=function()
	{
		...
		if(isIE)_jDoc.keydown(function(ev){var rng=_this.getRng();if(ev.which==8&&rng.item){$(rng.item(0)).remove();return false;}});//IE控件上Backspace會導致頁面後退
		...
		if(settings.html5Upload)jBody.bind('dragenter dragover',function(ev){if($.inArray('Files', ev.originalEvent.dataTransfer.types)!=-1)return false;}).bind('drop',function(ev){
			...
			if(dataTransfer&&(fileList=dataTransfer.files)&&fileList.length>0){
				...
				for(i in arrCmd){
					cmd=arrCmd[i];
					if(settings['up'+cmd+'Url'])arrExt.push(cmd+':,'+settings['up'+cmd+'Ext']);//允許上傳
				}
	...
	{
		var sHTML='<html><head>'+headHTML+'<title>預覽</title>'+(urlBase?'<base href="'+urlBase+'"/>':'')+'</head><body>' + _this.getSource() + '</body></html>';
